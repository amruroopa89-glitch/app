...
* Navigate to the test directory (`appium-tests`) and install dependencies:
  ...
  ```
  *(We recommend using a Python Virtual Environment like `python -m venv venv` and activating it before running `pip install`).*
...
appium
```
*(Keep this window open during execution).*
