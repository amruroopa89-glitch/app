...
export const Route = createFileRoute("/_authenticated/chat")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Chat with AI for crop, pest, fertilizer and irrigation guidance.",
      },
    ],
  }),
  component: ChatPage,
...
function ChatPage() {
  ...
  const [messages, setMessages] = useState<Msg[]>([
    {
      role: "assistant",
      content:
        "Hi! 👋 I'm your AI farming assistant. Ask me anything about crops, soil, pests, fertilizers or irrigation.",
    },
  ]);
  ...
  useEffect(() => {
    supabase
      .from("profiles")
      .select(
        "full_name,village,district,state,farm_size,farm_unit,soil_type,soil_ph,nitrogen,phosphorus,potassium,water_availability,irrigation_type,current_season,crop_history",
      )
      .maybeSingle()
      .then(({ data }) => {
        if (!data) return;
        setProfile({
          ...
          location:
            [data.village, data.district, data.state].filter(Boolean).join(", ") || undefined,
      ...
      <div className="space-y-3 pb-32">
        {messages.map((m, i) => (
          <div key={i} className={`flex ${m.role === "user" ? "justify-end" : "justify-start"}`}>
            <div
              className={`max-w-[85%] rounded-2xl px-4 py-2.5 text-sm shadow-[var(--shadow-card)] ${m.role === "user" ? "rounded-br-sm bg-primary text-primary-foreground" : "rounded-bl-sm bg-card text-foreground border border-border"}`}
            >
              {m.role === "assistant" && (
                <Sparkles className="mb-1 inline h-3.5 w-3.5 text-primary mr-1" />
              )}
      ...
      <div className="fixed bottom-20 left-0 right-0 z-40 border-t border-border bg-background/95 px-4 py-3 backdrop-blur">
        <div className="mx-auto flex max-w-2xl items-center gap-2">
          ...
          <button
            type="button"
            onClick={toggleVoice}
            className={`flex h-12 w-12 items-center justify-center rounded-full text-primary-foreground shadow-[var(--shadow-soft)] ${listening ? "animate-pulse" : ""}`}
          ...
          <button
            onClick={() => send(input)}
            disabled={loading}
            className="flex h-12 w-12 items-center justify-center rounded-full text-primary-foreground shadow-[var(--shadow-soft)] disabled:opacity-60"
