...
function RecommendPage() {
  ...
  return (
    <AppLayout variant="crops">
      ...
      <form
        onSubmit={submit}
        className="rounded-3xl border border-border bg-card p-4 shadow-[var(--shadow-card)] space-y-4"
      >
        <Select
          label="Soil Type"
          value={form.soilType}
        ...
        <Num
          ...
          value={form.soilPh}
          step={0.1}
          onChange={(v) => setForm({ ...form, soilPh: v })}
        />
        ...
        <Text
          label="Region (district, state)"
          value={form.region}
          onChange={(v) => setForm({ ...form, region: v })}
          placeholder="e.g. Anantapur, Andhra Pradesh"
        />
        <Text
          label="Recent crop history"
          value={form.history}
      ...
      {results && (
        <div className="mt-6 space-y-4">
          ...
          {results.map((c: any, i) => {
            ...
            const emoji = c?.emoji || "🌾";
            const scoreVal =
              typeof c === "object" && c?.score !== undefined ? parseFloat(c.score) : NaN;
            ...
            const tips =
              c?.tips ||
              c?.tip ||
              "Follow standard recommended management practices for best yield.";
            ...
            return (
              ...
              >
                <header
                  className="flex items-center gap-4 p-4 text-primary-foreground"
                  style={{ background: gradients[i % gradients.length] }}
                >
                  <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-white/95 text-4xl shadow">
                    {emoji}
                  </div>
