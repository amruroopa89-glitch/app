      ...
      if (parsed && (parsed.recommendations || Array.isArray(parsed))) {
        ...
        const CROP_DB: Record<string, any> = {
          ...
          Groundnut: {
            ...
            profit: "₹22,000/acre",
            demand: "Medium",
            tips: "Apply gypsum at 200 kg/acre at pegging stage.",
          },
          Maize: {
            emoji: "🌽",
            yield: "3.0 t/acre",
        ...
        const normalized = rawRecs.map((item: any, idx: number) => {
          let name =
            typeof item === "string" ? item : item?.name || item?.crop || `Crop ${idx + 1}`;
        ...
        return {
          ...
          rationale: parsed.rationale || "Recommended crops based on soil conditions.",
        };
      }
      return parsed;
    }
