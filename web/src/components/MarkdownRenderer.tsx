    ...
    <div className="markdown-content w-full text-inherit">
      <ReactMarkdown
        ...
        components={{
          ...
          thead: ({ ...props }) => (
            <thead className="bg-muted/70 text-foreground font-semibold" {...props} />
          ),
          tbody: ({ ...props }) => <tbody className="divide-y divide-border/60" {...props} />,
          th: ({ ...props }) => (
            <th
              className="px-3 py-2.5 font-bold border-b border-border text-foreground"
              {...props}
            />
          ),
          td: ({ ...props }) => <td className="px-3 py-2.5 text-muted-foreground" {...props} />,
          ul: ({ ...props }) => (
            <ul className="list-disc pl-5 my-2.5 space-y-1 text-sm" {...props} />
          ),
          ol: ({ ...props }) => (
            <ol className="list-decimal pl-5 my-2.5 space-y-1 text-sm" {...props} />
          ),
