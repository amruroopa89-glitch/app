...
export function createTestContext(driver) {
  ...
  const step = async (id, module, desc, action, expected, fn) => {
    ...
    try {
      const res = simMode ? (await sleep(25), `Simulated OK: ${expected}`) : await fn();
...
export async function runCategory(categoryName, testFn) {
  ...
  console.log("\n─────────────────────────────────────────────────────────");
  console.log(
    `[+] ${categoryName}: ${ctx.stepResults.length} steps | PASS: ${totalPass} | FAIL: ${totalFail}`,
  );
...
export async function generateCombinedReport(allResults) {
  ...
  const excelPath = path.join(
    reportsDir,
    `GreenHarvestBuddy_Selenium_E2E_${Math.floor(Date.now() / 1000)}.xlsx`,
  );
  ...
  console.log("\n═══════════════════════════════════════════════════════════");
  console.log(
    `[+] GRAND TOTAL: ${allSteps.length} steps | PASS: ${totalPass} | FAIL: ${totalFail}`,
  );
  console.log(`[+] Duration: ${((endTime - startTime) / 1000).toFixed(1)}s`);
