...
async function main() {
  ...
  for (let i = 0; i < 80; i++) {
    ...
    steps.push({
      id: `TC-WEB-${String(tcIndex++).padStart(3, "0")}`,
  ...
  const authScenarios = [
    ...
    "Signup Password Strength Limits",
    "Invalid Verification Email Links",
    "Forgotten Password Reset Token",
    "Database Duplicated Email check",
  ];
  const authChecks = [
  ...
  const securityThreats = [
    "SQL Injection on Chat Input",
    "XSS script tags in Profile Name",
    "CSRF Token tampering on form save",
    "JWT token header tampering",
    "Directory traversal on upload file path",
    "Brute-force password login limits",
    "Supabase Row Level Security policy",
    "CORS origin header validation",
    "Clickjacking frame-options headers",
    "Broken Object Level Authorization checking",
  ];
  const secScenarios = [
  ...
  const errorEvents = [
    ...
    "LocalStorage storage full",
    "Invalid API payload key",
  ];
  const errorChecks = [
  ...
  const sessionEvents = [
    "Supabase Session persistence reload",
    "Auth token timeout logout redirect",
    "LocalStorage credentials deletion",
    "Double login session termination",
    "Remember-Me token checking",
  ];
  const sessionChecks = [
