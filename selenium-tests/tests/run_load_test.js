...
async function main() {
  ...
  console.log(`Success RPS       : ${(successfulRequests / durationSec).toFixed(1)} req/sec`);
  console.log("----------------------------------------");
...
Success RPS       : ${(successfulRequests / durationSec).toFixed(1)} req/sec
----------------------------------------
Response Time:
...
`;
  ...
  addStep(
    "TC-LOAD-VAL-201",
    "Target URL Availability under Load",
    `GET ${targetUrl}`,
    "URL returns status 200",
  ...
  addStep(
    "TC-LOAD-VAL-202",
    "Concurrency Test",
    `Spawn ${concurrency} virtual workers`,
    "Workers complete requests without crash",
    `Successfully spawned and completed`,
    "PASS",
  );
  addStep(
    "TC-LOAD-VAL-203",
    "Average Latency Check",
    `Measure roundtrip response times`,
    "Average response time < 500ms",
    `Avg: ${avgResponseTime}ms | Min: ${minResponseTime}ms | Max: ${maxResponseTime}ms`,
    avgResponseTime < 500 ? "PASS" : "FAIL",
  );
  addStep(
    "TC-LOAD-VAL-204",
    "Throughput Level (RPS)",
    `Calculate requests per second`,
    "RPS > 10 req/sec",
    `${rps} req/sec`,
    parseFloat(rps) > 10 ? "PASS" : "FAIL",
  );
  addStep(
    "TC-LOAD-VAL-205",
    "Load Success Rate",
    `Verify status codes of all responses`,
    "Success rate > 95%",
    `${successRate}%`,
    parseFloat(successRate) > 95 ? "PASS" : "FAIL",
  );
