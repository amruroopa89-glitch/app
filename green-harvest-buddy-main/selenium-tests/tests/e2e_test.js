...
async function runTests() {
  ...
  await step('TC-UI-001','UI/UX','Landing page navbar renders','Locate <nav>','Navbar visible',
    async()=>{ const n=await waitForElement(driver,'nav'); return `Nav OK: "${(await n.getText()).slice(0,30)}"`;});
  ...
  await step('TC-FUNC-046','Profile','Select soil type Black','Select Black in soil dropdown','Black soil selected',
    async()=>{ const sels=await driver.findElements(By.css('select')); if(sels.length>0){ const o=await sels[0].findElement(By.css('option[value="Black"]')); await o.click(); } return 'Soil: Black';});
