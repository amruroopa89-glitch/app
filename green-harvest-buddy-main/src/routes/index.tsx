...
function Welcome() {
  return (
    <main className="relative min-h-screen overflow-hidden text-primary-foreground" style={{ background: "var(--gradient-welcome)" }}>
      ...
      <section className="relative mx-auto flex min-h-screen w-full max-w-3xl flex-col items-center px-5 pb-8 pt-10 text-center sm:pt-12">
        <header>
          ...
          <p className="mx-auto mt-5 max-w-xl text-base font-medium leading-relaxed sm:text-xl">
            Personalized crop recommendations, instant disease detection,<br className="hidden sm:block" /> and real-time market insights — all in your pocket.
          </p>
        </header>
        ...
        <div className="mt-2 flex w-full max-w-xl flex-col gap-3">
          <Button asChild size="lg" className="h-16 rounded-3xl bg-[var(--gradient-welcome-action)] text-lg font-bold shadow-[var(--shadow-welcome)] hover:opacity-95">
            <Link to="/auth" search={{ mode: "signup" }}>
              Get Started <ArrowRight className="h-5 w-5" />
            </Link>
          </Button>
          <Button asChild size="lg" variant="outline" className="h-14 rounded-3xl border-[var(--welcome-line)] bg-transparent text-base font-semibold text-primary-foreground shadow-none hover:bg-[var(--welcome-glow)] hover:text-primary-foreground">
            <Link to="/auth" search={{ mode: "signin" }}>Already have an account? Sign In</Link>
          </Button>
        </div>
